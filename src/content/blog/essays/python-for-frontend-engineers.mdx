...
### Start Here: The 2-Minute Python Checklist

**The Big Shift:** Python trades raw execution speed for developer productivity — it's [interpreted line-by-line by a runtime VM, not JIT-compiled](#pythons-paradox-slow-execution-fast-development) like modern JS engines, which is exactly why it dominates data science and AI (orchestrate in simple Python, call fast C/C++ libraries underneath) while staying rare in performance-critical backends. It's dynamically typed like JS, but *strongly* typed — Python never silently coerces types behind your back — and ships with a famously ["batteries included"](#the-python-standard-library-batteries-included) standard library, so you'll reach for `pip install` far less often than `npm install`.
...
**1. Core Syntax & Types**
* **[Naming Conventions](#1-naming-conventions-embrace-the-snake):** Python strictly uses `snake_case` for variables and functions, saying a complete goodbye to JavaScript's `camelCase`.
* **[Booleans & Null](#the-big-picture-python-vs-jsts):** Python embraces minimalism by replacing `null` and `undefined` with a solitary `None`, and requires `True`, `False`, and `None` to be [strictly capitalized](#2-the-capitalization-rule).
* **[Truthiness](#2-the-truthiness-shift-abandoning-length-checks) & [Equality](#1-strong-typing-and-the-absence-of-):** Python's `==` behaves exactly like JavaScript's strict `===`, and all empty collections (`[]`, `{}`, `""`) simply evaluate as Falsy.
...
* **[Variables & Scope](#3-the-scope-trap-lexical-scoping-and-the-read-only-illusion):** Because Python assumes you are creating local variables upon assignment (`=`), you must explicitly use the `nonlocal` or `global` keywords if you intend to modify state in an outer scope.
...
* **[Modules & Imports](#modules-the-perfect-11-mapping-to-es6):** Python's module system directly maps to ES6 Imports, but officially discourages wildcard imports (`from module import *`) to maintain strict namespace clarity.

**5. Files, Testing & Production Tooling**
...
* **[Testing](#the-industry-truth-everyone-actually-uses-pytest):** The standard library ships `unittest` (Jest-like, but class-based), but in practice almost everyone uses the leaner, function-based `pytest` instead.
...
This isn't "Python is secretly fast" — it's that Python hands the actually expensive work to something else. A plain Python loop still pays the interpreter's per-iteration cost, but a call into a compiled library doesn't:
...
| **Jython** | Java | Runs Python on the JVM, with direct access to Java libraries. |
| **IronPython** | C# / .NET | Same idea, for the .NET ecosystem. |
...
"Hashable" isn't limited to `int`/`float`/`str` — `bool` and tuples qualify too, and the reverse also holds: **dicts**, not just lists, can never be used as keys either, for the exact same "mutable" reason.
